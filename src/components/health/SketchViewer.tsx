...
export default function SketchViewer({ sketch }: { sketch: Sketch }) {
  ...
  useEffect(() => {
    ...
    catch { setError("3D needs WebGL on this device. You can still download the scene JSON."); return; }
    setError("");
    ...
    return () => { actions.current=null; observer.disconnect(); controls.dispose(); renderer.domElement.removeEventListener("webglcontextlost",lost); group.traverse(object => { if(object instanceof THREE.Mesh) { object.geometry.dispose(); object.material.dispose(); } }); renderer.dispose(); renderer.domElement.remove(); };
  },[sketch]);
  return <><div className="sketch-canvas" ref={host} role="img" aria-label={sketch.title} />{error && <p role="alert">{error}</p>}<div className="sketch-controls">{[["left","Rotate left"],["right","Rotate right"],["in","Zoom in"],["out","Zoom out"],["reset","Reset view"]].map(([action,label]) => <button type="button" key={action} onClick={() => actions.current?.move(action)}>{label}</button>)}</div><p className="sketch-hint">Drag to turn · Pinch to zoom</p><div className="sketch-controls"><button type="button" onClick={() => actions.current?.download()} disabled={!!error}>Download GLB</button><button type="button" onClick={() => download(new Blob([JSON.stringify(sketch,null,2)],{type:"application/json"}),"wolverine-sketch.json")}>Download scene JSON</button></div></>;
}
