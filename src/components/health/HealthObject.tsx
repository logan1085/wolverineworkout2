...
export default function HealthObject({id,title,description,onOpen,onAction,action,live=false}: {id:string;title:string;description:string;onOpen:(id:string)=>void;onAction?:()=>void;action?:string;live?:boolean}) {
  const asset=catalog.find(a=>a.id===id) || catalog[0];
  ...
  return <section className={`health-object ${live ? "health-object-live" : ""}`}>
    ...
    <div className="object-copy"><h3>{title}</h3><p>{description}</p><div className="object-actions">{onAction && <button className="light-btn" onClick={onAction}>{action}</button>}<button className="quiet-button" onClick={()=>onOpen(asset.id)}>Explore in 3D ↗</button></div></div>
  </section>;
}
