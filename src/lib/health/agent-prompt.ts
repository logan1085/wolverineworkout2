...
export const HEALTH_PROMPT_VERSION = "wolverine-health-2026-09-22.2";
...
const MEMORY_ON = `MEMORY IS ENABLED
...
export function buildHealthPrompt(options: {
  sample: boolean;
  ...
  return [
    ...
    HEALTH_RULES,
    options.sample
      ? "SAMPLE MODE: All supplied health records belong to a fictional sample profile, not the user. Call it the sample profile when referring to those records. Do not infer the user's health from them. Suggestions must be empty."
      : "PERSONAL MODE: Use only supplied personal context; do not invent an onboarding history.",
    options.memoryEnabled && !options.sample
      ? MEMORY_ON
      : "MEMORY IS OFF: Return an empty suggestions array. Do not promise future recall. You may use this conversation to answer now; do not imply persistent memory is available.",
...
Good reply begins: "Stop running and contact local emergency services now." Do not ask a question before that direction, diagnose the cause, or suggest another workout. Suggestions: [].`,
    options.sample
      ? "FINAL CHECK: Refer to the fictional sample profile, never to these records as the user's. Suggestions must be []."
    ...
    ...(!options.memoryEnabled || options.sample ? ["FINAL MEMORY-OFF CHECK: Suggestions must be []. If asked to remember, explicitly limit use to this conversation and state that future recall is unavailable while memory is off. No promise to keep anything in mind for next time or future chats."] : []),
    `Runtime prompt version: ${HEALTH_PROMPT_VERSION}. Current UTC date: ${date}.`,
  ].join("\n\n");
}

export function buildStravaPrompt() {
  return `${WOLVERINE_VOICE}
